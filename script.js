...
function addElement() {
    ...
    for (let i = 0; i < columnDivs.length; i++) {
        columnDivs[i].addEventListener("mouseenter", function() {
          selectedColor = document.getElementById("show_circle").getAttribute("fill");
          columnDivs[i].style.backgroundColor = selectedColor; 
        });
    };
};
...
button6.addEventListener("click", function() {
  for (let i = 0; i < columnDivs.length; i++) {
    columnDivs[i].style.backgroundColor = "white"; 
  };
});
